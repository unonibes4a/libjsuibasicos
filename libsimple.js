...
class InputSimple {
    constructor(parentHtml, idGenerico, labelText, inputValue = '', inputType = 'text', eventCallback = () => {}, inputClass = '', minValue = null, maxValue = null) {
        ...
        if (inputClass && inputClass.trim() !== '') {
            this.input.classList.add(inputClass.trim());
        } else {
...
class Btnormal {
    constructor(parentHtml, idGenerico, texto, eventClic = () => {}, forLabel = '') {
        this.css();
        this.label = document.createElement('label');
        this.label.classList.add('btnormal');
        this.label.id = `${idGenerico}btcircle`;
        this.label.innerHTML = `${texto}`;
        this.label.htmlFor = forLabel;
        parentHtml.appendChild(this.label);
        this.label.onclick = (e) => {
            if (eventClic) {
                eventClic(this.label, e);
            }
        }
    }

    css = () => {
        if (!document.getElementById("btnormalcss30dic")) {
            const styleTag = document.createElement('style');
            styleTag.id = "btnormalcss30dic";
            styleTag.textContent = `
            .btnormal {
                background: #313131;
                color: #ffffff;
...
class InputWithButtonControl {
    constructor(parentHtml, idGenerico, placeholder = '', buttonText = 'Subscribe', onInput = () => {}, onButtonClick = () => {}) {
        this.css();
        this.parentHtml = parentHtml;
        this.idGenerico = idGenerico;

        this.divWrapper = document.createElement('div');
        this.divWrapper.classList.add('input-wrapper');
        this.divWrapper.id = `${idGenerico}-wrapper`;
        ...
        this.button.classList.add('Subscribe-btn');
        this.button.innerHTML = buttonText;
        this.divWrapper.appendChild(this.button);

        this.parentHtml.appendChild(this.divWrapper);

        this.input.oninput = (e) => {
            onInput(this.input.value, this.input, e);
        };

        this.button.onclick = (e) => {
            onButtonClick(this.input.value, this.input, this.button, e);
        };
    }

    css = () => {
        if (!document.getElementById("input-with-button-css")) {
            const styleTag = document.createElement('style');
            styleTag.id = "input-with-button-css";
            styleTag.textContent = `
                .input-wrapper {
                  width: fit-content;
                  height: 50px;
...
class SimpleInputControl {
    constructor(parentHtml, idGenerico, labelText, inputValue = '', inputType = 'text', onInput = () => {}) {
        this.css();
        this.parentHtml = parentHtml;
        this.idGenerico = idGenerico;
        ...
        this.inputGroup.id = `${idGenerico}-group`;

        this.input = document.createElement('input');
        this.input.required = true;
        this.input.type = inputType;
        this.input.name = `${idGenerico}-input-name`;
        ...
        this.label = document.createElement('label');
        this.label.classList.add('user-label');
        this.label.htmlFor = `${idGenerico}-input`;
        this.label.textContent = labelText;

        this.inputGroup.appendChild(this.input);
        this.inputGroup.appendChild(this.label);
        this.parentHtml.appendChild(this.inputGroup);
        ...
        this.input.oninput = (e) => {
            ...
            onInput(valueToPass, this.input, e);
        };
    }

    css = () => {
        if (!document.getElementById("simple-input-css")) {
            const styleTag = document.createElement('style');
            styleTag.id = "simple-input-css";
            styleTag.textContent = `
                .input-group {
                    position: relative;
                    margin: 15px 0;
...
class SingleRadioButtonControl {
    constructor(parentHtml, idGenerico, name, labelText, value, isChecked = false, onChange = () => {}) {
        this.css();
        this.parentHtml = parentHtml;
        this.idGenerico = idGenerico;
        this.name = name;
        ...
        this.divRadioButton = document.createElement('div');
        this.divRadioButton.classList.add('radio-button');
        this.divRadioButton.id = `${idGenerico}-radio-button`;

        this.input = document.createElement('input');
        this.input.type = 'radio';
        this.input.name = this.name;
        this.input.id = `${this.idGenerico}-input`;
        this.input.classList.add('radio-button__input');
        this.input.value = this.value;
        this.input.checked = isChecked;

        this.label = document.createElement('label');
        this.label.htmlFor = `${this.idGenerico}-input`;
        this.label.classList.add('radio-button__label');
        this.label.textContent = labelText;

        this.customSpan = document.createElement('span');
        this.customSpan.classList.add('radio-button__custom');
        this.label.prepend(this.customSpan);

        this.divRadioButton.appendChild(this.input);
        this.divRadioButton.appendChild(this.label);
        this.parentHtml.appendChild(this.divRadioButton);

        this.input.onchange = (e) => {
            onChange(this.input.value, this.input.checked, this.input, e);
        };
    }

    css = () => {
        if (!document.getElementById("single-radio-button-css")) {
            const styleTag = document.createElement('style');
            styleTag.id = "single-radio-button-css";
            styleTag.textContent = `
                .radio-button {
                  display: inline-block;
                  position: relative;
